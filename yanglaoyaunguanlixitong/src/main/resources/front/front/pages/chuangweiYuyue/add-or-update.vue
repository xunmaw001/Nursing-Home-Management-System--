	...
	<view class="content">
		<form class="app-update-pv">
			...
			<view
				...
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
					class="title">床位</view>
				<picker @change="chuangweiChange" :value="chuangweiIndex" :range="chuangweiOptions"
					range-key="chuangweiName">
					<view
						:style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						class="uni-input">{{ruleForm.chuangweiId?ruleForm.chuangweiName:"请选择床位"}}</view>
				</picker>
			</view>
			<view
				:style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
				...
				<input disabled
					...
					:disabled="ro.chuangweiYuyueUuidNumber" type="text" v-model="ruleForm.chuangweiYuyueUuidNumber"
					placeholder="报名编号"></input>
			</view>
			<view
				...
				<textarea
					:style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
					:disabled="ro.chuangweiYuyueText" v-model="ruleForm.chuangweiYuyueText" placeholder="报名理由" />
			</view>
		...
		<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false"
			@confirm="chuangweiYuyueShenheTimeConfirm" ref="chuangweiYuyueShenheTime" themeColor="#333333"></w-picker>
	...
	export default {
		data() {
			return {
				...
				ruleForm: {
					chuangweiYuyueUuidNumber: this.getUUID(), //数字
					yonghuId: '',
					laorenId: '',
					chuangweiId: '',
					chuangweiYuyueText: '',
					insertTime: '',
					chuangweiYuyueYesnoTypes: '', //数字
					chuangweiYuyueYesnoValue: '', //数字对应的值
		...
		async onLoad(options) {
			...
			let chuangweiData = await this.$api.page(`chuangwei`, chuangweiParams);
			this.chuangweiOptions = chuangweiData.data.list;
			/*下拉框*/
			let chuangweiYuyueYesnoTypesParams = {
				page: 1,
				limit: 100,
				dicCode: 'chuangwei_yuyue_yesno_types',
			}
			let chuangweiYuyueYesnoTypes = await this.$api.page(`dictionary`, chuangweiYuyueYesnoTypesParams);
			this.chuangweiYuyueYesnoTypesOptions = chuangweiYuyueYesnoTypes.data.list
			...
			if (options.id) {
				...
				// 获取信息
				let res = await this.$api.info(`chuangweiYuyue`, this.ruleForm.id);
				this.ruleForm = res.data;
			}
			if (options.chuangweiYuyueId) {
				this.ruleForm.chuangweiYuyueId = options.chuangweiYuyueId;
			}
			// 跨表
			// this.styleChange()
		},
		methods: {
			...
			// 下拉变化
			chuangweiYuyueYesnoTypesChange(e) {
				this.chuangweiYuyueYesnoTypesIndex = e.target.value
				this.ruleForm.chuangweiYuyueYesnoValue = this.chuangweiYuyueYesnoTypesOptions[this
					.chuangweiYuyueYesnoTypesIndex].indexName
				this.ruleForm.chuangweiYuyueYesnoTypes = this.chuangweiYuyueYesnoTypesOptions[this
					.chuangweiYuyueYesnoTypesIndex].codeIndex
			},
			// styleChange() {
			// 	this.$nextTick(() => {
			...
			// 日期控件
			chuangweiYuyueShenheTimeConfirm(val) {
			...
			async onSubmitTap() {
				if ((!this.ruleForm.chuangweiYuyueUuidNumber)) {
					this.$utils.msg(`报名编号不能为空`);
					return
				}
				if ((!this.ruleForm.chuangweiYuyueText)) {
					this.$utils.msg(`报名理由不能为空`);
					return
				}
				if (this.ruleForm.id) {
					await this.$api.update(`chuangweiYuyue`, this.ruleForm);
				} else {
					await this.$api.save(`chuangweiYuyue`, this.ruleForm);
				}
