	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.chuangweiName" type="text" placeholder="床位名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('chuangwei','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('chuangwei','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='chuangweiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in chuangweiTypesList" :key="index" :class="chuangweiTypes===item.codeIndex?'tabActive':''"
                                  @tap="chuangweiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.chuangweiPhoto?baseUrl+product.chuangweiPhoto.split(',')[0]:''">
            </image>
            <view
                    ...
                    class="title">{{product.chuangweiName}}
            </view>
            <view v-if="product.chuangweiNewMoney"
                ...
                <text style="font-size: 35rpx;">￥</text>{{product.chuangweiNewMoney}}
            </view>
            <view v-else-if="product.chuangweiValue"
                  :style='{"padding":"10rpx 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                {{product.chuangweiValue}}
            </view>
            <view style="display: flex;justify-content: space-between;margin: 3%;">
                <text v-if="isAuth('chuangwei','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if=" isAuth('chuangwei','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 床位类型 字段 字典表查询方法
            async chuangweiTypesSelect(){
                let chuangweiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'chuangwei_types',
                });
                chuangweiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.chuangweiTypesList = chuangweiTypes.data.list
            },
            //当前表的 床位类型字段类别搜索
			chuangweiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.chuangweiTypes != 0) {
                    params.chuangweiTypes = this.chuangweiTypes
                }
                let _this = this
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`chuangwei`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.chuangweiName) {
                    searchForm['chuangweiName'] = this.searchForm.chuangweiName
                }

                let res = await this.$api.list(`chuangwei`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.chuangweiContent!=null){
                        item.chuangweiContent = item.chuangweiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
