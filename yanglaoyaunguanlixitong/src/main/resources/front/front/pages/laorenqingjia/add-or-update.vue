        ...
        <form class="app-update-pv">
            ...
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.laorenqingjiaUuidNumber" type="text" v-model="ruleForm.laorenqingjiaUuidNumber" placeholder="报名编号"></input>
            </view>
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.laorenqingjiaText" v-model="ruleForm.laorenqingjiaText" placeholder="报名理由"/>
            </view>
            ...
            <view class="btn">
                <button
					...
					<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="laorenqingjiaShenheTimeConfirm"
                              ref="laorenqingjiaShenheTime" themeColor="#333333"></w-picker>
...
<script>
    ...
    export default {
        data() {
            ...
            ruleForm: {
					laorenqingjiaUuidNumber: this.getUUID(),//数字
					jiashuId: '',
					laorenId: '',
					laorenqingjiaText: '',
					insertTime: '',
					laorenqingjiaYesnoTypes: '',//数字
					laorenqingjiaYesnoValue: '',//数字对应的值
		...
		/*下拉框*/
			let laorenqingjiaYesnoTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'laorenqingjia_yesno_types',
            }
			let laorenqingjiaYesnoTypes = await this.$api.page(`dictionary`, laorenqingjiaYesnoTypesParams);
			this.laorenqingjiaYesnoTypesOptions = laorenqingjiaYesnoTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`laorenqingjia`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.laorenqingjiaId){
                this.ruleForm.laorenqingjiaId = options.laorenqingjiaId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            // 下拉变化
            laorenqingjiaYesnoTypesChange(e) {
			...
			// 日期控件
			laorenqingjiaShenheTimeConfirm(val) {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.laorenqingjiaUuidNumber)) {
                    this.$utils.msg(`报名编号不能为空`);
                    return
                }
				if ((!this.ruleForm.laorenqingjiaText)) {
                    this.$utils.msg(`报名理由不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`laorenqingjia`, this.ruleForm);
                } else {
                    await this.$api.save(`laorenqingjia`, this.ruleForm);
                }
