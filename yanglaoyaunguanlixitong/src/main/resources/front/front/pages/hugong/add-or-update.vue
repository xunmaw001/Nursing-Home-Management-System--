        ...
        <form class="app-update-pv">
            ...
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.hugongUuidNumber" type="text" v-model="ruleForm.hugongUuidNumber" placeholder="报名编号"></input>
            </view>
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.hugongText" v-model="ruleForm.hugongText" placeholder="报名理由"/>
            </view>
            ...
            <view class="btn">
                <button
					...
					<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="hugongShenheTimeConfirm"
                              ref="hugongShenheTime" themeColor="#333333"></w-picker>
...
<script>
    ...
    export default {
        data() {
            ...
            ruleForm: {
					hugongUuidNumber: this.getUUID(),//数字
					jiashuId: '',
					laorenId: '',
					hugongText: '',
					insertTime: '',
					hugongYesnoTypes: '',//数字
					hugongYesnoValue: '',//数字对应的值
					hugongYesnoText: '',
					hugongShenheTime: '',
					createTime: '',
            },
		...
		/*下拉框*/
			let hugongYesnoTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'hugong_yesno_types',
            }
			let hugongYesnoTypes = await this.$api.page(`dictionary`, hugongYesnoTypesParams);
			this.hugongYesnoTypesOptions = hugongYesnoTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`hugong`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.hugongId){
                this.ruleForm.hugongId = options.hugongId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.hugongUuidNumber)) {
                    this.$utils.msg(`报名编号不能为空`);
                    return
                }
				if ((!this.ruleForm.hugongText)) {
                ...
                if (this.ruleForm.id) {
                    ...
                    await this.$api.save(`hugong`, this.ruleForm);
                }
