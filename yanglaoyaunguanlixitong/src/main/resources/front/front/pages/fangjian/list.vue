	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.fangjianName" type="text" placeholder="房间名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('fangjian','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('fangjian','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='fangjianTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in fangjianTypesList" :key="index" :class="fangjianTypes===item.codeIndex?'tabActive':''"
                                  @tap="fangjianTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.fangjianName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('fangjian','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.fangjianUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">房间编号</text>
                     <text style="width: 70%">
                         <text v-if="item.fangjianUuidNumber != null">{{item.fangjianUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.fangjianAddress" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">房间地点</text>
                     <text style="width: 70%">
                         <text v-if="item.fangjianAddress != null">{{item.fangjianAddress}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.fangjianValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">房间类型</text>
                     <text style="width: 70%">{{item.fangjianValue}}</text>
                 </view>
                <view v-show="item.fangjianContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">房间介绍</text>
                    <view style="width: 70%" v-html="item.fangjianContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('fangjian','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('fangjian','审核') && item.fangjianYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 房间类型 字段 字典表查询方法
            async fangjianTypesSelect(){
                let fangjianTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'fangjian_types',
                });
                fangjianTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.fangjianTypesList = fangjianTypes.data.list
            },
            //当前表的 房间类型字段类别搜索
			fangjianTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.fangjianTypes != 0) {
                    params.fangjianTypes = this.fangjianTypes
                }
                let _this = this
                let res = await this.$api.list(`fangjian`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('fangjian', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`fangjian`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.fangjianName) {
                    searchForm['fangjianName'] = this.searchForm.fangjianName
                }

                let res = await this.$api.list(`fangjian`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.fangjianContent!=null){
                        item.fangjianContent = item.fangjianContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
