        ...
        <form class="app-update-pv">
            ...
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.lizhiUuidNumber" type="text" v-model="ruleForm.lizhiUuidNumber" placeholder="报名编号"></input>
            </view>
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.lizhiText" v-model="ruleForm.lizhiText" placeholder="报名理由"/>
            </view>
            ...
            <view class="btn">
                <button
					...
					<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="lizhiShenheTimeConfirm"
                              ref="lizhiShenheTime" themeColor="#333333"></w-picker>
...
<script>
    ...
    export default {
        data() {
            ...
            ruleForm: {
					lizhiUuidNumber: this.getUUID(),//数字
					yonghuId: '',
					lizhiText: '',
					insertTime: '',
					lizhiYesnoTypes: '',//数字
					lizhiYesnoValue: '',//数字对应的值
					lizhiYesnoText: '',
					lizhiShenheTime: '',
					createTime: '',
            },
		...
		/*下拉框*/
			let lizhiYesnoTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'lizhi_yesno_types',
            }
			let lizhiYesnoTypes = await this.$api.page(`dictionary`, lizhiYesnoTypesParams);
			this.lizhiYesnoTypesOptions = lizhiYesnoTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`lizhi`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.lizhiId){
                this.ruleForm.lizhiId = options.lizhiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.lizhiUuidNumber)) {
                    this.$utils.msg(`报名编号不能为空`);
                    return
                }
				if ((!this.ruleForm.lizhiText)) {
                    this.$utils.msg(`报名理由不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`lizhi`, this.ruleForm);
                } else {
                    await this.$api.save(`lizhi`, this.ruleForm);
                }
